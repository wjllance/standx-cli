...
def markdown_panel(content: str) -> dict[str, Any]:
    return {
        ...
        "layout": {"x": 0, "y": 47, "w": 192, "h": 6, "i": 11},
        "htmlContent": "",
...
ORDER BY _timestamp DESC LIMIT 1''',
        ...
        markdown_panel(
            "**PnL scope:** values are emitted by the maker session. Starting inventory is external to the session, so its acquisition cost is not part of maker-session PnL. Use the Inventory and Events panels to evaluate it."
        ),
    ]
...
ORDER BY _timestamp DESC LIMIT 200''',
                [axis("_timestamp", "Time"), axis("action", "Action"), axis("kind", "Kind"), axis("severity", "Severity"), axis("event", "Event"), axis("request_id", "Request"), axis("side", "Side"), axis("reason", "Reason"), axis("message", "Message")],
