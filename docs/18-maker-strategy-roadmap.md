...
| 4 | alpha | 漂移感知报价（v0 mark 动量非对称） | drift 控制器、非对称 spread/skew 派生 | 负向 markout 显著改善，成交结构与运维门槛不恶化 |
...
- 离线信号定价（2026-07-18，train/val 严格切分，见 evidence 归因补充）给 v0 两条硬
  ...
  加宽——**A/B 必须增设无条件加宽对照臂**，把"漂移条件化"与"加宽本身"分离，否则
  改善无法归因；其二，初始参数取训练窗调参区 T=30–60s、k≈1.0–1.5、cap=8bps，后续
  调参只准在训练窗口进行，验收用冻结参数 + 样本外窗口（统一验收口径）。
