...
| 4 | alpha | 漂移感知报价（v0 mark 动量非对称，前置零代码加宽 A/B） | 加宽 A/B 报告、drift 控制器、非对称派生 | 加宽 A/B 先定方向；漂移臂须同时胜过 baseline 与恒宽臂 |
...
控制器，减少高库存持续时间和被动积累后的退出成本。2026-07-18 的 markout 曲线（同一批
阶段 2 A/B 数据，见其 evidence）显示成交后出血在 ~60s 内饱和（mo30 -8.9 → mo60 -9.8 →
mo300 -9.4bps，neg% 92→71），不存在持续的库存盯市恶化：快速部分退出 ≈ 在底部实现亏损，
持有不额外付费。本阶段定位因此明确为库存**尾部**风险治理（趋势市满仓 + stop-loss 残余
敞口的截断），而非逐笔 MTM 止血；优先级维持在阶段 4 之后。

### 范围

- 基线为阶段 4 accepted 后的冻结配置；若阶段 4 未 accepted 则仍为静态 baseline（见
  统一验收口径的基线继承规则）。
