...
| 订单回报流（`ws-api/v1`） | 正常 live maker 直接发送签名 `order:new`/`order:cancel`，并按 `request_id` 关联接受/拒绝回报 | REST 继续负责快照、清理和恢复；XAG-USD 已取得完整生产 canary 证据，重新锁定的 live gate 等待 release-owner 审核决定 |
...
该传输变更重新锁定 live gate。2026-07-14 的前两次受控生产记录证明了 WS 命令提交和
故障后清理，但没有保留关联的 venue acceptance 与正常 cancel 完整证据。隐藏 canary
随后加固为输出 create/cancel request ID、venue order ID、REST 可见/消失和最终零仓位的
结构化证据，并在 XAG-USD 最小数量监督实验中完整通过。该结果解决了传输证据缺口；
是否重新解锁仍需 release owner 明确审核决定，不能由 canary 自动推定。
